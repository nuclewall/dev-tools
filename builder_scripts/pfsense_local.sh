...
if [ ! -f ${BUILD_CONF} ]; then
	echo "cannot find pfsense-build.conf"
fi
...
export PRODUCT_NAME=${PRODUCT_NAME:-nuclewall}
...
export PFSENSEBASEDIR=${PFSENSEBASEDIR:-/usr/local/nucle-fs}

# Directory that FreeSBIE will clone to in order to create
# iso staging area.
export PFSENSEISODIR=${PFSENSEISODIR:-/usr/local/nucle-clone}
...
export BASEDIR=${PFSENSEBASEDIR:-/usr/local/nucle-fs}
export CLONEDIR=${PFSENSEISODIR:-/usr/local/nucle-clone}
...
if [ ${FREEBSD_VERSION} -gt 7 -a "$FREEBSD_BRANCH" != "RELENG_8_1" ]; then
	export MODULES_OVERRIDE=${MODULES_OVERRIDE:-"i2c ipmi acpi ndis ipfw ipdivert dummynet fdescfs cpufreq opensolaris zfs glxsb runfw if_stf virtio coretemp amdtemp aesni"}
else
...
export ISOPATH=${ISOPATH:-${MAKEOBJDIRPREFIXFINAL}/${PRODUCT_NAME}-${PFSENSE_VERSION}-${ARCH}-`date '+%d-%m-%Y'`_${PFSENSETAG}.iso}
export IMGPATH=${IMGPATH:-${MAKEOBJDIRPREFIXFINAL}/${PRODUCT_NAME}-${PFSENSE_VERSION}-${ARCH}-`date '+%Y%m%d-%H%M'`_${PFSENSETAG}.img}
export MEMSTICKPATH=${MEMSTICKPATH:-${MAKEOBJDIRPREFIXFINAL}/${PRODUCT_NAME}-memstick-${PFSENSE_VERSION}-${ARCH}-`date '+%Y%m%d-%H%M'`_${PFSENSETAG}.img}
export MEMSTICKSERIALPATH=${MEMSTICKSERIALPATH:-${MAKEOBJDIRPREFIXFINAL}/${PRODUCT_NAME}-memstick-serial-${PFSENSE_VERSION}-${ARCH}-`date '+%Y%m%d-%H%M'`_${PFSENSETAG}.img}
